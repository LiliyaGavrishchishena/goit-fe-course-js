...
function Cashier(name, productsDatabase) {
  ...
  this.countTotalPrice = function(order) {
    const purchases = Object.entries(order);
    const prices = Object.entries(this.productsDatabase);
    for (let i = 0; i < purchases.length; i += 1) {
      for (let j = 0; j < prices.length; j += 1) {
        if (purchases[i][0] === prices[j][0]) {
          this.totalPrice += purchases[i][1] * prices[j][1];
        }
      }
    }
  };
