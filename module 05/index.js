...
// */
const getAllSkills = arr => arr.reduce( (accx, x) => x.skills ? accx.concat(x.skills) : accx, [] ).reduce( (accy, y) => !accy.includes(y) ? accy.concat(y) : accy, [] ).sort();
...
// */
const getUserNamesSortedByFriendsCount = arr => arr.sort( (a, b) => a.friends.length - b.friends.length ).map(x => x.name);
